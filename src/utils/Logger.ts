...
class Logger implements ILogger {
  ...
  private getReadableTime = (now: Date): string => `${now.getHours()}:${now.getMinutes()}.${now.getSeconds()}`;
  ...
  private isLogLevelEnabled = (logLevel: LOG_LEVELS): boolean => VERBOSITY_LEVELS[this.logLevel] >= VERBOSITY_LEVELS[logLevel];
}
